...
插件会持续采集真实项目协作的两条信息线：用户提出的目标、约束、决策、修正与偏好，以及 Agent 完成的分析、实现、变更与当前进度。每条记录都会保留说话主体和来源，TMCRA 服务在后台继续整理已经提交的证据。

随着项目推进，系统可以自动沉淀：

- 项目记忆：需求、决策、里程碑、当前状态、故障记录和待解决问题；
- 可复用经验：概念、方法、解释、研究笔记和经验教训；
- 个人上下文：用户明确表达的个人资料、偏好和人物关系；
- 可查看的记忆与知识图谱（`Visual Atlas`）和个人知识库。每条知识仍可追溯到原始证据，并区分“已确认、暂定、已替代、待解决”等状态。

用户可以在 TMCRA 网页控制台和桌面应用中查看这些结果。图谱与知识库会随着记忆变化重新生成，同时保留原始记录。Harness 插件负责采集、召回、注入和写回，后续的图谱构建与知识整理由 TMCRA 服务完成。
...
- 可视化图谱和个人知识库目前在 TMCRA 客户端中查看，本技术预览尚未把这些查看器嵌入 Harness。
